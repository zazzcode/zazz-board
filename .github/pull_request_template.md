<!--
PR template for the zazz-board repo.
...
Use the repo's actual system of record for this change (Zazz Board deliverable/task, GitHub issue, etc.).
Do not dump every related link into the PR.
If the final URL or ID is not yet available, leave a clear placeholder such as `TODO: add tracker URL`.
If the PR is draft, make that obvious here and briefly state what remains before the Deliverable Owner can mark it ready for formal review.
